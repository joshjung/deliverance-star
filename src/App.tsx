...
import type { ContentTreeNode, Footnote, Theme } from "./types/types";
import "./App.css";

const THEME_KEY = "bookTheme";
const FONT_SIZE_KEY = "bookFontSize";
...
export default function App() {
	...
	const [showLocationSavedToast, setShowLocationSavedToast] =
	...
	const locationSavedToastTimeoutRef = useRef<ReturnType<
		typeof setTimeout
	> | null>(null);
	...
	const renderFontSizeIndicator = (): JSX.Element => {
		return (
			<span className="font-size-indicator">
				<span
					className={`font-size-a ${fontSize === "small" ? "font-size-active" : ""}`}
					style={{ fontSize: "1em" }}
				>
					a
				</span>
				<span
					className={`font-size-a ${fontSize === "medium" ? "font-size-active" : ""}`}
					style={{ fontSize: "1.2em" }}
				>
					a
				</span>
				<span
					className={`font-size-a ${fontSize === "large" ? "font-size-active" : ""}`}
	...
	useEffect(() => {
		...
		const handleScroll = (): void => {
			...
			if (hash) {
				...
				if (
					highlightedElement &&
					highlightedElement.classList.contains("paragraph-with-anchor")
				) {
	...
	useEffect(() => {
		const handleFootnoteInteraction = (
		...
		const handleParagraphClick = (event: MouseEvent): void => {
			...
			if (
				target.classList.contains("paragraph-anchor") ||
				target.classList.contains("footnote-ref")
			) {
				return;
			}
		...
		if (contentElement) {
			...
			contentElement.addEventListener("touchstart", handleFootnoteTouch, {
				passive: false,
			});
	...
	return (
		<div className="app">
			...
			/>
			<div
				className={`top-left-controls ${isMenuOpen ? "top-left-controls-hidden" : !controlsVisible ? "top-left-controls-auto-hidden" : ""}`}
			>
				<button
				...
				<button
					...
					aria-label={`Font size: ${fontSize}. Click to toggle.`}
				>
					{renderFontSizeIndicator()}
				</button>
				<button
				...
				<div className="reading-progress">{Math.round(readingProgress)}%</div>
			</div>
			<div
				className={`book-content ${activeFootnote ? "book-content-dimmed" : ""}`}
				ref={contentRef}
			></div>
			...
			{showLocationSavedToast && (
				<div className="location-saved-toast">
					Book location is saved automatically as you scroll. You can safely
